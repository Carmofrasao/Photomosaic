...
//função que imprime o fotomosaico no formato P3 na saida padrão
{
	...
	for(int lp = 0; lp < alt; lp++)
	{
		for(int cp = 0; cp < larg; cp++)
		{
			fprintf(saida, "%d %d %d ", mosaico[lp][cp].r, mosaico[lp][cp].g, mosaico[lp][cp].b);
		}
		fprintf(saida, "\n");
	}
}
...
//função qu preenche a matriz que se tornara o fotomosaico no formato P3
{
	...
	for(int lpr = 0; lpr < ima_princi.alt-30; lpr += vpasti[1].alt)
	{
		...
		{
			cor_pred_p3(ima_princi, vpasti[1].larg, vpasti[1].alt, cpr, lpr, &auxr, &auxg, &auxb);
...
//função que aloca memoria para a matriz do fotomosaico no formato P3, e inicia a impreção das caracteristicas do mesmo
{
	...
	m = malloc (ima_princi.alt * sizeof (rgb_p3*));
	if(m == NULL)
	{
	...
	{
		m[i] = malloc (ima_princi.larg * sizeof (rgb_p3));
		if(m[i] == NULL)
		{
			perror ("Memoria insuficiente");
		}
	...
	zerar_fotomosaico_p3(m, ima_princi.larg, ima_princi.alt);
	preenche_matriz_p3(vpasti, ima_princi, m, num_past);
	
	//iniciando a criacao do fotomosaico
	fprintf(saida, "%s\n", ima_princi.tipo);
	fprintf(saida, "%d %d\n", ima_princi.larg, ima_princi.alt);
	fprintf(saida, "%d\n", ima_princi.max);
	imprimir_fotomosaico_p3(m, ima_princi.larg, ima_princi.alt, saida);
	for(int i = 0; i < ima_princi.alt; i++)
	{
		free(m[i]);
	}
